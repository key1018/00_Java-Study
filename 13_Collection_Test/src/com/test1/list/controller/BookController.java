...
public class BookController {
	 ...
	 * @return
	 */
	public ArrayList<Book> insertBook(String title, String author, int category, int price) {
		/*
		...
		case 3 : str = "의료"; break;
		case 4 : str = "기타"; break;
		default : System.out.println("장르르 다시 선택해주세요."); break;
		}
	 ...
	 * 5_1. 수정 전,후 도서 출력 요청
	 */
	public ArrayList<Book> updateBook(String title) {

		ArrayList<Book> update = new ArrayList<>();

		for (int i = 0; i < bookList.size(); i++) {
			Book b = bookList.get(i);
			if ((b.getTitle()).equals(title)) {
				update.add(bookList.get(i));
				break;
			}
		}
		return update;
	 ...
	 * 5_2. 도서 수정용 요청 처리
	 */
	public int updateBook(String title, String update) {
		...
		for(int i = 0; i < bookList.size(); i++) {
			...
			if((b.getTitle()).equals(title)) {
				...
				b.setCategory(st.nextToken());

				price = Integer.parseInt(st.nextToken()); // 문자열로 출력되므로 int형으로 반환시키기
				b.setPrice(price);
